...
var ErrExitNodeLocalIP = errors.New("cannot use this address as an exit node as it is a local IP address to this machine")
...
func checkIPNotSelf(st *ipnstate.Status, ip netaddr.IP) bool {
	for _, tip := range st.TailscaleIPs {
		if ip == tip {
			return false
		}
	}
...
// ClearExitNodeIP sets the ExitNodeIP to the zero value.
func (p *Prefs) ClearExitNodeIP() {
